defmodule TradingDesk.DecisionsLive do
  ...
  Traders can:
    - See all proposed, applied, rejected, revoked decisions
    - Apply a proposed decision to the shared effective state
    - Reject a proposed decision
    - Revoke their own decision (proposed or applied)
    - See conflicts when two decisions touch the same variables
    - Filter by product group and status
  """
  ...
  def handle_event("switch_trader", %{"trader" => tid}, socket) do
    ...
    socket =
      socket
      |> assign(:trader_id, if(trader, do: trader.id, else: nil))
  ...
  def handle_info({:effective_state_changed, _}, socket) do
    {:noreply, refresh_effective_state(socket)}
  end
      ...
      <div style="display:grid;grid-template-columns:280px 1fr;height:calc(100vh - 45px)">
        ...
        <div style="overflow-y:auto;padding:20px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:16px">
            <%= for {label, val} <- [{"ALL", "all"}, {"PROPOSED", "proposed"}, {"APPLIED", "applied"}, {"REJECTED", "rejected"}, {"REVOKED", "revoked"}, {"SUPERSEDED", "superseded"}] do %>
          ...
          <% else %>
            <%= for decision <- @decisions do %>
              ...
              <div style={"background:#{status_bg(decision.status)};border:1px solid #{if conflicts, do: "#f59e0b", else: "#1e293b"};border-radius:8px;padding:16px;margin-bottom:12px;#{if conflicts, do: "box-shadow:0 0 12px rgba(245,158,11,0.15);", else: ""}"}>
                <%!-- Header row --%>
                ...
                <div style="display:flex;gap:8px;margin-top:8px">
                  ...
                  <% end %>
                  <%= if decision.status in ["proposed", "applied"] and decision.trader_id == @trader_id do %>
