defmodule TradingDesk.ContractsLive do
        ...
        <div style="display:flex;align-items:center;gap:8px">
          <%= for pg <- @product_groups do %>
            <button phx-click="switch_product_group" phx-value-pg={pg}
              style={"padding:4px 12px;border-radius:4px;font-size:11px;font-weight:600;cursor:pointer;border:1px solid #{if @product_group == pg, do: "#38bdf8", else: "#1e293b"};background:#{if @product_group == pg, do: "#0c4a6e", else: "transparent"};color:#{if @product_group == pg, do: "#38bdf8", else: "#94a3b8"}"}>
      ...
      <div style="background:#0d1117;border-bottom:1px solid #1b2838;padding:0 20px;display:flex;gap:2px">
        <%= for {role, label, icon, color} <- [{:trader, "Trader", "📊", "#38bdf8"}, {:legal, "Legal", "⚖️", "#a78bfa"}, {:operations, "Operations", "🏭", "#f59e0b"}] do %>
          <button phx-click="switch_role" phx-value-role={role}
            style={"padding:10px 20px;border:none;font-size:12px;font-weight:600;cursor:pointer;background:#{if @role == role, do: "#111827", else: "transparent"};color:#{if @role == role, do: color, else: "#7b8fa4"};border-bottom:2px solid #{if @role == role, do: color, else: "transparent"}"}>
            <%= icon %> <%= label %>
          </button>
        <% end %>
      </div>
      ...
      <div style="display:grid;grid-template-columns:400px 1fr;height:calc(100vh - 100px)">
        ...
        <div style="background:#0a0f18;border-right:1px solid #1b2838;overflow-y:auto;padding:14px">
          ...
          <div style="margin-bottom:16px;padding:12px;background:#111827;border-radius:8px">
            <div style="font-size:11px;color:#94a3b8;letter-spacing:1px;margin-bottom:8px">INGEST CONTRACT</div>
          ...
          <%!-- Contract list --%>
          <div style="font-size:11px;color:#94a3b8;letter-spacing:1px;margin-bottom:8px">
            CONTRACTS — <%= @product_group |> to_string() |> String.upcase() %>
            <span style="color:#7b8fa4">(<%= length(@contracts) %>)</span>
          </div>
          <%= for contract <- @contracts do %>
            <div phx-click="select_contract" phx-value-id={contract.id}
              ...
              <div style="display:flex;justify-content:space-between;align-items:center">
                ...
                <span style={"font-size:12px;font-weight:700;padding:2px 6px;border-radius:3px;background:#{status_bg(contract.status)};color:#{status_color(contract.status)}"}>
              ...
              <div style="display:flex;gap:6px;margin-top:3px;font-size:12px">
                <span style={"color:#{if contract.sap_validated, do: "#10b981", else: "#94a3b8"}"}><%= if contract.sap_validated, do: "SAP✓", else: "SAP?" %></span>
                ...
                <%= if contract.company do %>
                  <span style="color:#7b8fa4"><%= fmt_company(contract.company) %></span>
                <% end %>
  ...
  defp render_trader_panel(assigns) do
    ...
    <div style="background:#111827;border-radius:10px;padding:20px;margin-bottom:16px">
      ...
      <%= if @gate_report do %>
        ...
        <div style="font-size:11px;color:#94a3b8;letter-spacing:1px;margin-bottom:8px">CONTRACT GATE STATUS</div>
        <table style="width:100%;border-collapse:collapse;font-size:11px;margin-bottom:16px">
          <thead><tr style="border-bottom:1px solid #1e293b">
            <th style="text-align:left;padding:4px;color:#94a3b8">Counterparty</th>
            <th style="text-align:left;padding:4px;color:#94a3b8">Type</th>
            <th style="text-align:center;padding:4px;color:#94a3b8">G1</th>
            <th style="text-align:center;padding:4px;color:#94a3b8">G2</th>
            <th style="text-align:center;padding:4px;color:#94a3b8">G3</th>
            <th style="text-align:right;padding:4px;color:#94a3b8">Blockers</th>
          </tr></thead>
          <tbody>
            <%= for cr <- @gate_report.contracts do %>
              <tr style="border-bottom:1px solid #1e293b11">
                ...
                <td style="padding:4px;color:#94a3b8"><%= fmt_atom(cr.template_type) %> <%= fmt_atom(cr.incoterm) %></td>
        ...
        <%= if length(@gate_report.master_blockers) > 0 do %>
          <div style="font-size:11px;color:#94a3b8;letter-spacing:1px;margin-bottom:8px">MASTER GATE BLOCKERS</div>
    ...
    <%= if @currency_report do %>
      <div style="background:#111827;border-radius:10px;padding:16px;margin-bottom:16px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <span style="font-size:11px;color:#94a3b8;letter-spacing:1px">DATA CURRENCY</span>
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px">
          ...
          <div style="background:#0a0f18;padding:8px;border-radius:4px;text-align:center">
            <div style="font-size:12px;color:#94a3b8">Stale</div>
          ...
          <div style="background:#0a0f18;padding:8px;border-radius:4px;text-align:center">
            <div style="font-size:12px;color:#94a3b8">PG Refresh</div>
    ...
    <div style="background:#111827;border-radius:10px;padding:16px;margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <span style="font-size:11px;color:#94a3b8;letter-spacing:1px">CONTRACT IMPACT PREVIEW</span>
        <button phx-click="preview_constraints"
          style="padding:4px 10px;border:1px solid #1e293b;border-radius:4px;font-size:12px;background:transparent;color:#a78bfa;cursor:pointer">
          PREVIEW
        </button>
      </div>
      <%= if @constraint_preview && length(@constraint_preview) > 0 do %>
        <table style="width:100%;border-collapse:collapse;font-size:11px">
          <thead><tr style="border-bottom:1px solid #1e293b">
            <th style="text-align:left;padding:4px;color:#94a3b8">Counterparty</th>
            <th style="text-align:left;padding:4px;color:#94a3b8">Parameter</th>
            <th style="text-align:right;padding:4px;color:#94a3b8">Current</th>
            <th style="text-align:center;padding:4px;color:#94a3b8">Op</th>
            <th style="text-align:right;padding:4px;color:#94a3b8">Contract</th>
            <th style="text-align:right;padding:4px;color:#94a3b8">Applied</th>
          </tr></thead>
          <tbody>
            <%= for p <- @constraint_preview do %>
              <tr style={"border-bottom:1px solid #1e293b11;background:#{if p.would_change, do: "#1a1a2e", else: "transparent"}"}>
                ...
                <td style="padding:4px;text-align:center;color:#94a3b8"><%= p.operator %></td>
                <td style="padding:4px;text-align:right;font-family:monospace;color:#a78bfa"><%= format_val(p.clause_value) %></td>
                <td style={"padding:4px;text-align:right;font-family:monospace;font-weight:#{if p.would_change, do: "700", else: "400"};color:#{if p.would_change, do: "#f59e0b", else: "#94a3b8"}"}><%= format_val(p.proposed_value) %></td>
              </tr>
            <% end %>
          </tbody>
        </table>
      <% else %>
        <div style="font-size:12px;color:#7b8fa4;font-style:italic">Click PREVIEW to see how contracts modify solver inputs</div>
      <% end %>
    </div>
    """
  end
  ...
  defp render_legal_panel(assigns) do
    ...
    <%= if @selected_contract do %>
      ...
      <div style="background:#111827;border-radius:10px;padding:20px;margin-bottom:16px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div>
            ...
            <%= if c.company do %>
              <div style="font-size:11px;color:#7b8fa4;margin-top:2px"><%= TemplateRegistry.company_label(c.company) %></div>
            <% end %>
          </div>
        ...
        <%= if c.template_validation do %>
          <div style="margin-top:16px;padding:12px;border-radius:6px;background:#0a0f18;border:1px solid #1e293b">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
              <span style="font-size:11px;color:#94a3b8;letter-spacing:1px">TEMPLATE COMPLETENESS</span>
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;font-size:11px">
              <div style="text-align:center"><span style="color:#94a3b8">Required</span><br/><span style={"font-weight:700;color:#{if c.template_validation.required_met == c.template_validation.required_total, do: "#10b981", else: "#ef4444"}"}><%= c.template_validation.required_met %>/<%= c.template_validation.required_total %></span></div>
              <div style="text-align:center"><span style="color:#94a3b8">Expected</span><br/><span style={"font-weight:700;color:#{if c.template_validation.expected_met == c.template_validation.expected_total, do: "#10b981", else: "#f59e0b"}"}><%= c.template_validation.expected_met %>/<%= c.template_validation.expected_total %></span></div>
              <div style="text-align:center"><span style="color:#94a3b8">SAP</span><br/><span style={"font-weight:700;color:#{if c.sap_validated, do: "#10b981", else: "#94a3b8"}"}><%= if c.sap_validated, do: "✓", else: "?" %></span></div>
              <div style="text-align:center"><span style="color:#94a3b8">Blocks</span><br/><span style={"font-weight:700;color:#{if c.template_validation.blocks_submission, do: "#ef4444", else: "#10b981"}"}><%= if c.template_validation.blocks_submission, do: "YES", else: "NO" %></span></div>
            </div>
            <%!-- Template findings --%>
            <%= if length(c.template_validation.findings) > 0 do %>
              <div style="margin-top:10px">
                <%= for f <- c.template_validation.findings do %>
                  <div style={"padding:4px 8px;margin-bottom:2px;font-size:12px;border-left:2px solid #{finding_color(f.level)};color:#{finding_color(f.level)}"}>
                    <%= f.message %>
                  </div>
                <% end %>
              </div>
        ...
        <%= if c.llm_validation do %>
          <div style="margin-top:8px;padding:8px 12px;border-radius:6px;background:#0a0f18;border:1px solid #1e293b;font-size:11px">
            <span style="color:#94a3b8">LLM Verify:</span>
            <span style={"font-weight:600;color:#{if c.llm_validation.errors > 0, do: "#ef4444", else: "#10b981"}"}><%= c.llm_validation.errors %> errors</span>,
            <span style={"color:#{if c.llm_validation.warnings > 0, do: "#f59e0b", else: "#94a3b8"}"}><%= c.llm_validation.warnings %> warnings</span>
          </div>
        <% end %>
        ...
        <%= if c.status == :draft do %>
          <div style="margin-top:16px">
            <button phx-click="submit_for_review" phx-value-id={c.id}
              style={"padding:8px 20px;border:none;border-radius:4px;font-weight:600;font-size:12px;cursor:pointer;background:#{if c.template_validation && c.template_validation.blocks_submission, do: "#1e293b", else: "#7c3aed"};color:#{if c.template_validation && c.template_validation.blocks_submission, do: "#94a3b8", else: "#fff"}"}>
      ...
      <div style="background:#111827;border-radius:10px;padding:16px">
        <div style="font-size:11px;color:#94a3b8;letter-spacing:1px;margin-bottom:10px">EXTRACTED CLAUSES (<%= length(c.clauses || []) %>)</div>
          ...
          <div style={"padding:12px;margin-bottom:6px;border-radius:6px;background:#0a0f18;border-left:3px solid #{confidence_color(clause.confidence)}"}>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
              ...
              <div style="display:flex;gap:6px;align-items:center">
                <span style={"font-size:12px;padding:2px 6px;border-radius:3px;background:#{confidence_bg(clause.confidence)};color:#{confidence_color(clause.confidence)}"}>
                  <%= clause.confidence %>
                </span>
      ...
      <div style="background:#111827;border-radius:10px;padding:20px;margin-bottom:16px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:16px">
          <div style={"background:#0a0f18;padding:10px;border-radius:6px;text-align:center;border:1px solid #{if c.sap_validated, do: "#166534", else: "#1e293b"}"}>
            <div style="font-size:12px;color:#94a3b8">SAP Validated</div>
            <div style={"font-size:16px;font-weight:700;color:#{if c.sap_validated, do: "#10b981", else: "#94a3b8"}"}><%= if c.sap_validated, do: "YES", else: "NO" %></div>
          </div>
          <div style="background:#0a0f18;padding:10px;border-radius:6px;text-align:center">
            <div style="font-size:12px;color:#94a3b8">Open Position</div>
            <div style="font-size:16px;font-weight:700;color:#38bdf8"><%= if c.open_position, do: "#{format_number(c.open_position)}t", else: "—" %></div>
          </div>
          <div style="background:#0a0f18;padding:10px;border-radius:6px;text-align:center">
            <div style="font-size:12px;color:#94a3b8">Discrepancies</div>
        ...
        <%= if c.sap_discrepancies && length(c.sap_discrepancies) > 0 do %>
          <div style="font-size:11px;color:#94a3b8;letter-spacing:1px;margin-bottom:8px">DISCREPANCIES</div>
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            <thead><tr style="border-bottom:1px solid #1e293b">
              <th style="text-align:left;padding:4px;color:#94a3b8">Field</th>
              <th style="text-align:left;padding:4px;color:#94a3b8">Severity</th>
              <th style="text-align:right;padding:4px;color:#94a3b8">Contract</th>
              <th style="text-align:right;padding:4px;color:#94a3b8">SAP</th>
              <th style="text-align:left;padding:4px;color:#94a3b8">Message</th>
            </tr></thead>
            <tbody>
              <%= for d <- c.sap_discrepancies do %>
                <tr style="border-bottom:1px solid #1e293b11">
                  ...
                  <td style="padding:6px 4px;color:#94a3b8;font-size:12px"><%= d.message %></td>
                </tr>
              <% end %>
            </tbody>
          </table>
        ...
        <div style="font-size:11px;color:#94a3b8;letter-spacing:1px;margin-top:16px;margin-bottom:8px">EXTRACTED COMMERCIAL TERMS</div>
        <table style="width:100%;border-collapse:collapse;font-size:11px">
          <thead><tr style="border-bottom:1px solid #1e293b">
            <th style="text-align:left;padding:4px;color:#94a3b8">Type</th>
            <th style="text-align:left;padding:4px;color:#94a3b8">Parameter</th>
            <th style="text-align:center;padding:4px;color:#94a3b8">Op</th>
            <th style="text-align:right;padding:4px;color:#94a3b8">Value</th>
            <th style="text-align:left;padding:4px;color:#94a3b8">Unit</th>
            <th style="text-align:center;padding:4px;color:#94a3b8">Conf</th>
          </tr></thead>
          <tbody>
            <%= for clause <- (c.clauses || []) do %>
              <tr style="border-bottom:1px solid #1e293b11">
  ...
  defp confidence_color(_), do: "#94a3b8"
